<template>
  <div class="earlyWithdrawalContainer"> 
		  ...
		  <div class="tableContent" :style="{width:computed}">
			  <el-table :data="tableData3"  tooltip-effect="dark"  style="width: 100%"   height="600"  stripe   
				  ...
				  <el-table-column  label="序号" type="index"></el-table-column>
					...
					<el-table-column  label="原金额" show-overflow-tooltip>
					  <template slot-scope="scope">{{ scope.row.name }}</template>
					</el-table-column>
					<el-table-column  label="现金额" show-overflow-tooltip>
					  <template slot-scope="scope">{{ scope.row.name }}</template>
					</el-table-column>
					<el-table-column  label="状态" show-overflow-tooltip>
					  <template slot-scope="scope">{{ scope.row.name }}</template>
					</el-table-column>
					<el-table-column  label="笔数" show-overflow-tooltip>
					  <template slot-scope="scope">{{ scope.row.name }}</template>
					</el-table-column>
...
//					{name:'me'}
		...
		methods:{
			...
			popNow(row){
                this.SimplifiedVerRouter.push(this.tagsLists,this.tagActive,'/main/earlyWithdrawal/detail',{id:1});  
			},
